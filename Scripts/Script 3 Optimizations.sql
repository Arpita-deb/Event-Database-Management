...
--Updating the WeekendFlag in Calendar table
UPDATE [EventSphere Database].dbo.Calendar
SET
WeekendFlag = 
		CASE 
...
-- Test Query: Events occuring on weekends

SELECT event_ID, start_date, YearNum, MonthName, DayofMonth, DayOfWeekName
FROM Event E
JOIN Calendar C ON E.start_date = C.DateValue
WHERE C.WeekendFlag = 1;
...
-- Test Query
SELECT Event_ID, Event_type, Start_date, Estimated_budget, Total_expenditure, Status, Estimated_attendance, Actual_attendance, Capacity, Attendee_Name, Email, Ticket_type, Price
...
-- Test Query : Employees who haven't completed the tasks
SELECT 
	Event_ID, 
	Employee_name, 
	Task
...
-- Test Query: Events and venues with attendee counts exceeding 80% of capacity.
SELECT * 
FROM 
	[dbo].[VenueCapacityLimits]
WHERE 
	[Percent_of_Venue_capacity] > 80
ORDER BY 
...
-- Test Query: Tasks with a deadline less than or equal to 7 days
SELECT *
FROM 
	[dbo].[DeadlineTracker]
WHERE
...
Suggestion: Use a table-valued function (TVF) to return the event_ID, host organization, total revenue from tickets along with its start and end date.
*/


CREATE FUNCTION dbo.ufn_EventsByEventType(@EventType NVARCHAR(15)) -- takes an event_type as input
...
SELECT DISTINCT Host,
	...
	[Percent_of_Overall_Revenue] = FORMAT(SUM(Total_revenue_from_tickets) OVER(PARTITION BY Host) / SUM(Total_revenue_from_tickets) OVER(), 'p')
...
-- Test Query: Partners with a total event budget exceeding $200,000
...
-- Test Query: Top 3 Ticket types by Average price and number of tickets sold per event
